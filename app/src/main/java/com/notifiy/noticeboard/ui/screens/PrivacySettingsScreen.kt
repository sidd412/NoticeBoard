...
fun PrivacySettingsScreen(navController: NavController) {
        ...
        if (shouldRequestExport && currentUser != null) {
            try {
                ...
                if (result.isSuccess) {
                    ...
                    Toast.makeText(
                        context,
                        "Failed to submit export request: ${result.exceptionOrNull()?.message}",
                        Toast.LENGTH_LONG
                    ).show()
                }
            } catch (e: Exception) {
                Toast.makeText(
                    context,
                    "Error submitting export request: ${e.message}",
                    Toast.LENGTH_LONG
                ).show()
            } finally {
    ...
    LaunchedEffect(accountDeleted) {
        if (accountDeleted) {
            ...
            Toast.makeText(
                context,
                "Account deleted successfully. Restarting app...",
                Toast.LENGTH_LONG
            ).show()
    ...
    LazyColumn(
        modifier = Modifier
            .fillMaxSize(),
        contentPadding = PaddingValues(16.dp, vertical = 40.dp),
